...
// 			let filteredOArt = allData.filter((dataItem) => {
// 				return dataItem.authors.toLowerCase().includes(this.state.search.toLowerCase())
				
// 			});

// let filteredOArt = allData.pickBy(
...
// const originalArticles = filteredOArt.map((dataItem, index) => {
// 					return <div className='origAContainer' key={index}><li><h5>{dataItem.authors} {dataItem.description} <i>{dataItem.journal}</i> {dataItem.date} {dataItem.journalID}</h5></li></div>
// 				});
